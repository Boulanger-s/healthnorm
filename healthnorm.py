...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if enabled["swap_percent"]: h["swap_percent"]    = swap_percent()
        if enabled["disk_percent"]: h["disk_percent"]    = disk
        if enabled["disk_iops"]:    h["disk_iops"]       = disk_iops()
        if enabled["net_bytes"]:    h["net_bytes_sec"]   = net_bytes()
        if enabled["net_conns"]:    h["tcp_connections"] = net_conns()
        if enabled["net_errors"]:   h["net_errors"]      = net_errors()
        if enabled["cpu_temp"]:     h["cpu_temp_c"]      = cpu_temp()
        if enabled["kernel"]:       h["kernel"]          = open("/proc/sys/kernel/osrelease").read().strip()
        if enabled["timestamp"]:    h["timestamp"]       = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        if enabled["errors"]:       h["errors"]          = []
